...
def create_struture(num_function):
    function = f"function_{num_function}"
    structure = {
        project_name : {'functions' : { function : ['handler.py'] },
                        'shared' : ['db_connection.py', 'utils.py', 'auth.py', 'exceptions.py'],
                        'config' : ['serverless.yml', 'iam_roles.yml', 'env.yml', 'resources/'],
