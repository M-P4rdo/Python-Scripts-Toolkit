...
def create_struture(num_plugin):
    plugin = f"plugin_{num_plugin}"
    structure = {
        project_name : {'core' : ['plugin_loader.py', 'app.py', 'exceptions.py'],
                        'plugins': { plugin : ['logic.py', 'config.json'] },
                        'interfaces' : ['base_plugin.py'],
                        'config' : ['settings.py', 'plugins.yaml', 'logging_config.py']
                        }
    }
    return structure
...
if __name__ == "__main__":
    ...
    for i in range(0, number_plugins):
